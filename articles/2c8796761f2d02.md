---
title: "GraphQL Fragment Colocationでコンポーネントにデータ宣言を移行する"
emoji: "🐧"
...
例えば `<Friend>` の `friend` で現在は使用していない `age` というフィールドが必要になった時、`<Friend>` と親コンポーネントを書き換える必要があります。
...
この状況で先ほどのように `age` を `<Friend>` に追加してみましょう。
...
1ファイルの修正で実装が完了しました。
Friendの関心が `Friend` コンポーネントにまとまっているため、非常に分かりやすいと思います。
